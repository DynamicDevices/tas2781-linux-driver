				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (pFw_hdr->ndev != tas_dev->ndev) {
		if (n + 65 < size) {
			n  += scnprintf(buf + n, size  - n, "ERROR: device is NOT "
				"same between DTS(%u) and DSP(%u) firmware!\n\r",
	...
	if (n + 320 < size) {
		n  += scnprintf(buf + n, size  - n,
			...
			tas_dev->dsp_binaryname, pFirmware->bKernelFormat == true ?
			"Kernel" : "Git",
	...
	for (i = 0; i < pFirmware->mnPrograms; i++) {
		...
		for (j = 0; j < 64; j++) {
			if (n + 2 < size) {
				n  += scnprintf(buf + n, size - n, "%c", pProgram->mpName[j]);
			} else {
				scnprintf(buf + PAGE_SIZE - 100, 100, "\n[SmartPA - %d]:%s "
					"Out of memory!\n\r", __LINE__, __func__);
				n = PAGE_SIZE;
	...
	for (i = 0; i < pFirmware->mnConfigurations; i++) {
		if (n + 16 < size) {
			n  += scnprintf(buf + n, size - n, "\tnConfig:%d\n\r", i);
		}  else {
			scnprintf(buf + PAGE_SIZE - 100, 100, "\n[SmartPA - %d]:%s "
				"Out of memory!\n\r", __LINE__, __func__);
			n = PAGE_SIZE;
		...
		if (n + 19 < size) {
			n  += scnprintf(buf + n, size  - n, "\tProgram:0x%02x\n\r",
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if (tas_dev != NULL) {
		...
		n_result = tasdevice_dev_read(tas_dev,
			...
			pSysCmd->mnReg), &data);
		if (n_result < 0) {
			len  += scnprintf(buf, size - len, "[tasdevice]reg_show: "
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (count >= 20) {
		...
		if (n_result < 0) {
			pSysCmd->bufLen  += snprintf(gSysCmdLog[RegSettingCmd],
				256 - pSysCmd->bufLen,
				"[tasdevice]reg: write chn%dB0x%02xP0x%02xR0x%02x "
				"failed.\n\r", kbuf[0], kbuf[1], kbuf[2], kbuf[3]);
		} else {
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (tas_dev != NULL) {
		...
		if (pSysCmd->bCmdErr == true) {
			len = scnprintf(buf, PAGE_SIZE, gSysCmdLog[RegDumpCmd]);
			goto out;
		}
		//20 bytes
		if (len + 20 <= size) {
